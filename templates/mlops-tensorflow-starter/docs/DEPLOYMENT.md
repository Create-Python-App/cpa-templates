...
- **FastAPI**: `uv run uvicorn mlops_tensorflow.serving.app:app`, then
...
MlflowClient(tracking_uri=tracking_uri).set_registered_model_alias(
    "mlops-tensorflow-local", "production", "<version>"  # replace <version> with the actual number
)
```
...
Not required. This template is CPU-first: it depends on `tensorflow-cpu`,
so no CUDA-only dependency is ever required for `uv sync` or tests to
pass. For GPU training, replace `tensorflow-cpu` with `tensorflow` in
`pyproject.toml` (same Python API) and re-run `uv sync`; see the
TensorFlow GPU guide for driver requirements.
