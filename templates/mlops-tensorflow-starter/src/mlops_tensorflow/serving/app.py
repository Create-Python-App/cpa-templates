...
are not yet compatible with this template's `mlops-tensorflow` type):
...
from mlops_tensorflow.models.build import NormalizedMLP
from mlops_tensorflow.tracking.model_registry import load_model

logger = logging.getLogger(__name__)

app = FastAPI(title="mlops-tensorflow-starter serving")
...
def _model_uri() -> str:
    return os.environ.get("MODEL_URI", "models:/mlops-tensorflow-local@production")


@lru_cache(maxsize=8)
def _load_cached_model(model_uri: str) -> NormalizedMLP:
    ...
    # the cache (fixing the reload cost), a different URI is a fresh miss.
    # lru_cache does not cache exceptions, so a failed load is retried on
    # the next request rather than being stuck.
    #
...
def predict_endpoint(body: PredictRequest) -> PredictResponse:
    ...
    try:
        matrix = np.asarray(body.features, dtype=np.float32)
        if matrix.ndim != 2 or 0 in matrix.shape:
            raise ValueError("features must be a non-empty 2D matrix")
        predictions = np.argmax(np.asarray(model.predict(matrix, verbose=0)), axis=1)
    except Exception:
