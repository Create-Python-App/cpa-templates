...
Use this checklist before calling the TensorFlow MLOps starter "mature" for CPA.
...
- [x] CPU-first: depends on `tensorflow-cpu`, so no CUDA-only dependency is ever required for `uv sync` or tests to pass (swap in `tensorflow` for GPU training)
...
- [x] `[tool.mypy]` (with per-module `ignore_missing_imports` overrides for `tensorflow`/`mlflow`) and `[tool.pyright]` configured in `pyproject.toml`
...
`models:/mlops-tensorflow-local@production`. Promote manually once satisfied
...
| `mlops-tensorflow-distributed` | Distributed training for TensorFlow |
...
uvx create-awesome-python-app@latest my-pipeline \
  --template mlops-tensorflow-starter \
