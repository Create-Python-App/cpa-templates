...
are not yet compatible with this template's `mlops-pytorch` type):
...
from mlops_pytorch.models.build import NormalizedMLP
from mlops_pytorch.tracking.model_registry import load_model

logger = logging.getLogger(__name__)

app = FastAPI(title="mlops-pytorch-starter serving")
...
def _model_uri() -> str:
    return os.environ.get("MODEL_URI", "models:/mlops-pytorch-local@production")
...
def predict_endpoint(body: PredictRequest) -> PredictResponse:
    ...
    try:
        features = torch.as_tensor(np.asarray(body.features), dtype=torch.float32)
