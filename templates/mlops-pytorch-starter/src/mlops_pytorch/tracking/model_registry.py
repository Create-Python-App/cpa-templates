...
def register_model_version(model: torch.nn.Module, name: str, run_id: str) -> str:
    ...
    (`models/train.py` passes `run.info.run_id`).
    """
    _ = run_id
    info = mlflow.pytorch.log_model(model, artifact_path="model", registered_model_name=name)
