...
Use this checklist before calling the PyTorch MLOps starter "mature" for CPA.
...
- [x] `[tool.mypy]` (with per-module `ignore_missing_imports` overrides for `torch`/`mlflow`) and `[tool.pyright]` configured in `pyproject.toml`
...
`models:/mlops-pytorch-local@production`. Promote manually once satisfied
...
| `mlops-pytorch-distributed` | Distributed training for PyTorch |
...
uvx create-awesome-python-app@latest my-pipeline \
  --template mlops-pytorch-starter \
