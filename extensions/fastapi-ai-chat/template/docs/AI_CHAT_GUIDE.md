...
The **fastapi-ai-chat** extension adds a minimal `/chat` endpoint backed by
LangChain's provider abstraction (`langchain-core`'s `BaseChatModel`), with a
mock provider by default so it works offline with no API key. This
establishes the pattern future FastAPI AI extensions build on — it
intentionally does not include RAG, MCP, MLflow tracing, or a frontend.
...
| `app/features/chat/router.py` | `POST /chat` (not auto-mounted — see Usage) |
...
The router ships unmounted, matching `fastapi-auth-jwt`'s pattern. Add it to
`app/api/router.py`:
...
2. After mounting the router, `POST /api/v1/chat` with a `user` message returns a 200 with an `assistant` message.
...
|---------|--------------|-----|
| `404` on `/chat` | Router not mounted | `include_router(chat_router)` in `app/api/router.py`, per Usage |
| `500 unknown AI_CHAT_PROVIDER` | Typo or unimplemented provider name | Use `mock`, or wire a real provider via `langchain`'s `init_chat_model()` yourself |
