...
*   **`app.features.rag.schemas`**: Pydantic models defining the Input/Output boundaries (`DocumentMetadata`, `RetrievedDocument`).
...
*   `RAG_EMBEDDING_PROVIDER`: The embedding provider to use (default: `mock`). Set to `openai` to use real embeddings.
*   `RAG_EMBEDDING_MODEL`: The model name to use (default: `mock-embed`).
...
```
*(If using `fastapi-sqlalchemy`, add a `Document` model and generate a migration).*
